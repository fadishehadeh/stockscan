...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6 border-b border-gray-200">
        <nav class="flex space-x-8" role="tablist">
            <button type="button" onclick="window.switchTab('application')" data-tab="application" class="tab-button py-3 px-1 border-b-2 border-orange-600 font-medium text-gray-900 whitespace-nowrap">
                Application Settings
            </button>
            @if (auth()->user()->isSuperAdmin())
                <button type="button" onclick="window.switchTab('administration')" data-tab="administration" class="tab-button py-3 px-1 border-b-2 border-transparent font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap">
                    Administration
                </button>
            @endif
            <button type="button" onclick="window.switchTab('account')" data-tab="account" class="tab-button py-3 px-1 border-b-2 border-transparent font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap">
...
window.switchTab = function(tabName) {
    ...
    buttons.forEach(btn => {
        if (btn.dataset.tab === tabName) {
            ...
            btn.classList.add('border-orange-600', 'text-gray-900');
        } else {
            btn.classList.add('border-transparent', 'text-gray-500');
            btn.classList.remove('border-orange-600', 'text-gray-900');
        }
    });
